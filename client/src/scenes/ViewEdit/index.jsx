...
const ViewEdit = () => {
  ...
  const doa = record[0].date_of_assessment;
  const pgdate = record[0].previous_glass_prescription_date;
  const date = getCurrentTimestamp();
  const updateRecord = async (values, onSubmitProps) => {
    const formData = new FormData();
    for (let value in values) {
      if(value === "date_of_assessment" || value === "previous_glass_prescription_date"){
                ...
                <div>
                  <FlexBetween m="1.25rem 0.25rem 2rem">
                    ...
                    <FlexBetween gap="30px">
                      <TextField
                        label="DATE"
                        defaultValue={record[0].previous_glass_prescription_date}
                        name="date"
                        sx={{ gridColumn: "span 1" }}
                      />
                      <IconButton>
